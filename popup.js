...
function DownloadItem(data) {
    ...
    item.getElement('by-ext').onclick = function (event) {
        chrome.tabs.create({
            url: 'chrome://extensions#' + item.byExtensionId
        });
        event.stopPropagation();
        return false;
    }
...
DownloadItem.prototype.render = function () {
    ...
    item.getElement('myprogress').style.display = (
    ...
    if (item.totalBytes && (item.state != 'complete')) {
        item.getElement('myprogress').innerText = (
            item.getElement('complete-size').innerText + ' of ' +
...
DownloadManager.showNew = function () {
    ...
    //console.log('any hidden:'+any_hidden);
            ...
            $('#items').contextMenu({
                ...
                items: {
                    ...
                    "retry": {
                        ...
                        callback: function (key, options) {
                            chrome.tabs.create({
                                url: DownloadManager.getItem($(this)[0].id.substr(4)).url
                            });
                        }
                    },
                    "copy": {
                    ...
                    "sep2": "---------",
                    "delete": {
                        name: chrome.i18n.getMessage("contextMenuDeleteFromDisk"),
                        callback: function (key, options) {
...
document.addEventListener('keydown', function (event) {
    if (event.keyCode == 13 && event.ctrlKey) {
        ...
        if (selectedItem === undefined || selectedItemIndex === undefined) { //or greater or less than the size
            selectedItemIndex = 0;
        ...
        if (selectedItem === undefined || selectedItemIndex === undefined) { //or greater or less than the size
            ...
            selectedItemIndex = 0;
        } else if (selectedItemIndex == items.length - 1) {} else selectedItemIndex++;
